(function(){
  ...
  var app = angular.module('tone-timer.services', ['ionic']);
  var audioHash = {};


  // for media plugin : http://plugins.cordova.io/#/package/org.apache.cordova.media
  app.factory('AudioService', function($q, $ionicPlatform, $window){
    var service = {
      loadMedia: loadMedia,
      playMedia: playMedia,
    ...
    function loadMedia(src, onStop, onError, onStatus){
      ...
      $ionicPlatform.ready(function(){
        ...
        // play through cordova
          ...
          if(!audioHash[src]){
            audioHash[src] = new $window.Media(src, mediaSuccess, mediaError, mediaStatus);
          }

          audioHash[src].status = mediaStatus;
          defer.resolve(audioHash[src]);
        }
      });
